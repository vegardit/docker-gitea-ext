...
base_image=${DOCKER_BASE_IMAGE:-gitea/gitea:1}
...
declare -A image_meta=(
  ...
  [title]="$image_repo"
  [description]="Official Gitea Docker image enhanced with built-in AsciiDoc rendering powered by AsciiDoctor"
...
#################################################
# determine effective Gitea version
#################################################
echo
...
# extend tags
#################################################
#tags+=("${gitea_version%.*}.x")  #1.12.x
tags+=("${gitea_version%%.*}.x")  #1.x
